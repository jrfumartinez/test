...
#define ALGLIB_NO_EXPENSIVE_XTESTS
...
{
public:
    static bool runtest(bool verbose)
    {
        verbose_mode() = verbose;
        try
        {
            alglib::setnworkers(0);
            const alglib::ae_int_t ncores = alglib::getmaxnworkers();
            alglib::xparams parallel_callbacks = combine_params(alglib::parallel, alglib::parallel_callbacks);
            log("ncores=" + to_string_int(ncores));
            if (ncores <= 1)
                return true;

            log("running parallel case");
            case_result parallel_case = run_concurrency_case(ncores, parallel_callbacks);
            log("parallel: calls=" + to_string_int(parallel_case.total_calls) +
                ", max_active=" + to_string_int(parallel_case.max_active_callbacks) +
                ", bad_idx=" + to_string_int(parallel_case.bad_worker_indices));
            if (!validate_parallel_callbacks(parallel_case))
                return false;

            log("running serialized case");
            case_result serialized_case = run_concurrency_case(ncores, alglib::parallel);
            log("serialized: calls=" + to_string_int(serialized_case.total_calls) +
                ", max_active=" + to_string_int(serialized_case.max_active_callbacks) +
                ", bad_idx=" + to_string_int(serialized_case.bad_worker_indices));
            if (!validate_serialized_callbacks(serialized_case))
                return false;

            log("running exception case");
            if (!validate_callback_exception_propagation(ncores))
                return false;
        ...
        {
            log(std::string("top-level ap_error: ") + e.msg);
            return false;
        }
        catch(const std::exception &e)
        {
            log(std::string("top-level exception: ") + e.what());
            return false;
        }
        catch(...)
        {
            log("top-level non-std exception");
            return false;
        }
    }
...
private:
    ...
    static const int spin_milliseconds = 5;

    struct probe_state
    {
        explicit probe_state(alglib::ae_int_t ncores, bool throw_in_callback) :
            worker_seen(new std::atomic<int>[static_cast<std::size_t>(ncores)]),
            worker_count(ncores),
            active_callbacks(0),
            max_active_callbacks(0),
            total_calls(0),
            bad_worker_indices(0),
            throw_armed(throw_in_callback ? 1 : 0)
        {
            for(alglib::ae_int_t i=0; i<ncores; ++i)
                worker_seen[static_cast<std::size_t>(i)].store(0);
        }
        ...
        std::atomic<int> *worker_seen;
        alglib::ae_int_t worker_count;
        std::atomic<int> active_callbacks;
        std::atomic<int> max_active_callbacks;
        std::atomic<int> total_calls;
        std::atomic<int> bad_worker_indices;
        std::atomic<int> throw_armed;
    };

    struct case_result
    {
        explicit case_result(alglib::ae_int_t ncores) :
            worker_seen(static_cast<std::size_t>(ncores), 0),
            worker_count(ncores),
            max_active_callbacks(0),
            total_calls(0),
            bad_worker_indices(0)
        ...
        std::vector<int> worker_seen;
        alglib::ae_int_t worker_count;
        int max_active_callbacks;
        int total_calls;
        int bad_worker_indices;
    };
    ...
    static std::string to_string_int(long long value)
    {
    ...
    static std::string safe_message(const alglib::ap_error &e)
    {
        return e.msg;
    }

    static const char *callback_exception_needle()
    {
        return "cbck-test-message";
    }

    static alglib::xparams combine_params(const alglib::xparams &a, const alglib::xparams &b)
    {
        alglib::xparams tmp = a;
        return tmp | b;
    }

    static alglib::real_1d_array build_initial_point()
    {
    ...
    static void configure_solver(alglib::minlpsolverstate &state, alglib::ae_int_t ncores)
    {
        ...
        alglib::minlpsolversetbc(state, bndl, bndu);
        for(int i=0; i<n; ++i)
            alglib::minlpsolversetintkth(state, i);
        alglib::minlpsolversetalgobbsync(state, ncores);
        alglib::minlpsolverfavorinternalparallelism(state);
        alglib::minlpsolvernoadaptiveinternalparallelism(state);
        alglib::minlpsolversettimeout(state, timeout_seconds);
    }

    static void spin_at_least(int milliseconds)
    {
        volatile int sink = 0;
        const alglib::ae_int_t start = alglib_impl::ae_tickcount();
        for(;;)
        {
            for(int i=0; i<2000; ++i)
                sink += i;
            const alglib::ae_int_t now = alglib_impl::ae_tickcount();
            const alglib::ae_int_t elapsed = now - start;
    ...
    static void update_maximum(std::atomic<int> &target, int value)
    {
        int snapshot = target.load();
        while (snapshot < value && !target.compare_exchange_weak(snapshot, value))
        {
        }
    }

    static void objective_jacobian(const alglib::real_1d_array &x,
                                   alglib::real_1d_array &fi,
                                   alglib::real_2d_array &jac,
                                   void *ptr)
    {
        probe_state *probe = static_cast<probe_state*>(ptr);
        const alglib::ae_int_t worker_idx = alglib::getcallbackworkeridx();
        probe->total_calls.fetch_add(1);
        if (worker_idx >= 0 && worker_idx < probe->worker_count)
            probe->worker_seen[static_cast<std::size_t>(worker_idx)].store(1);
        else
            probe->bad_worker_indices.fetch_add(1);

        const int active_now = probe->active_callbacks.fetch_add(1) + 1;
        update_maximum(probe->max_active_callbacks, active_now);

        try
        {
            int expected = 1;
            if (worker_idx > 0 && probe->throw_armed.compare_exchange_strong(expected, 0))
            {
                throw std::runtime_error(std::string(callback_exception_needle()) + " worker=" + to_string_int(worker_idx));
            }

            spin_at_least(spin_milliseconds);
            double value = 0.0;
        ...
        {
            probe->active_callbacks.fetch_sub(1);
            throw;
        }
        probe->active_callbacks.fetch_sub(1);
    }

    static case_result run_concurrency_case(alglib::ae_int_t ncores, const alglib::xparams &xparams)
    {
        probe_state probe(ncores, false);
        alglib::minlpsolverstate state;
        case_result result(ncores);
        alglib::real_1d_array x0 = build_initial_point();

        alglib::minlpsolvercreate(x0, state);
        configure_solver(state, ncores);
        alglib::minlpsolveroptimize(state, objective_jacobian, NULL, &probe, xparams);

        for(alglib::ae_int_t i=0; i<ncores; ++i)
            result.worker_seen[static_cast<std::size_t>(i)] = probe.worker_seen[static_cast<std::size_t>(i)].load();
        result.max_active_callbacks = probe.max_active_callbacks.load();
        result.total_calls = probe.total_calls.load();
        result.bad_worker_indices = probe.bad_worker_indices.load();
        return result;
    }

    static bool all_workers_seen(const case_result &result)
    {
        for(alglib::ae_int_t i=0; i<result.worker_count; ++i)
            if (result.worker_seen[static_cast<std::size_t>(i)] == 0)
                return false;
        return true;
    }

    static bool validate_parallel_callbacks(const case_result &result)
    {
        return result.total_calls > 0 &&
               result.bad_worker_indices == 0 &&
               all_workers_seen(result) &&
               result.max_active_callbacks > 1;
    }

    static bool validate_serialized_callbacks(const case_result &result)
    {
        return result.total_calls > 0 &&
               result.bad_worker_indices == 0 &&
               all_workers_seen(result) &&
    ...
    static bool validate_callback_exception_propagation(alglib::ae_int_t ncores)
    {
        probe_state probe(ncores, true);
        alglib::minlpsolverstate state;
        alglib::real_1d_array x0 = build_initial_point();
        alglib::minlpsolvercreate(x0, state);
        configure_solver(state, ncores);
        ...
        {
            alglib::xparams parallel_callbacks = combine_params(alglib::parallel, alglib::parallel_callbacks);
            alglib::minlpsolveroptimize(state, objective_jacobian, NULL, &probe, parallel_callbacks);
            log("exception propagation failed: optimize returned normally, calls=" + to_string_int(probe.total_calls.load()));
            return false;
        }
        catch(const alglib::ap_error &e)
        {
            log("exception message=" + safe_message(e));
            return true;
        }
        catch(const std::exception &e)
        {
            log(std::string("exception propagation failed: non-ALGLIB std::exception, calls=") +
                to_string_int(probe.total_calls.load()) + ", what=" + e.what());
            return false;
        }
        catch(...)
        {
            log("exception propagation failed: non-ALGLIB unknown exception");
            return false;
        }
    }
};
...
#error Unable to determine OS, unexpected here
    ...
    printf("Issues:\n");
    {
        ...
        try
        {
            ...
            alglib_impl::ae_smart_ptr_init(&ptr0, (void**)&p0, ae_false, &_alglib_env_state, true);
            alglib_impl::ae_shared_pool_init(&pool, &_alglib_env_state, true);
            ...
            memset(&seed, 0, sizeof(seed));
            alglib_impl::ae_smart_ptr_init(&ptr0, (void**)&p0, ae_false, &_alglib_env_state, true);
            alglib_impl::ae_smart_ptr_init(&ptr1, (void**)&p1, ae_false, &_alglib_env_state, true);
            alglib_impl::ae_shared_pool_init(&pool, &_alglib_env_state, true);
            _seedrec_init(&seed, &_alglib_env_state, true);
            alglib_impl::ae_shared_pool_set_seed(&pool, &seed, sizeof(seed), _seedrec_init_copy, _seedrec_destroy, &_alglib_env_state);
            alglib_impl::ae_shared_pool_retrieve(&pool, &ptr0, &_alglib_env_state);
    ...
    fflush(stdout);
    {
        ...
        //
        #if defined(_ALGLIB_FULLSET)
        test_failed = !minlp_test::runtest(false);
        printf(fmt_str, "* minlp", test_failed ? "FAILED" : "OK");
        fflush(stdout);
        if( test_failed )
            return 1;
        #else
        printf(fmt_str, "* minlp", "n/a");
        fflush(stdout);
        #endif
    }
